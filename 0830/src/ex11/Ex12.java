...
class SutdaDeck {
	...
	void registerJokbo() {
		/*
		 * (1) 아래의 로직에 맞게 코드를 작성하시오. 1. jokbo(HashMap)에 족보를 저장한다. 두 카드의 값을 문자열로 붙여서
		 * key로, 점수를 value로 저장한다.
		 */
		jokbo.put("KK", 4000);
		jokbo.put("1010", 3100);
		jokbo.put("99", 3090);
	...
	int getPoint(Player p) {
		...
		if (c1.isKwang && c2.isKwang) { // 1
			...
			if (jokbo.containsKey(key)) {// 2
				result = jokbo.get(key);
			} else {// 3
				result = (c1.num + c2.num) % 10 + 1000;
			}
//			result = jokbo.get(""+c1.num+c2.num);//2
//			if(result==null){
//				result = (c1.num + c2.num) % 10+1000;//3
//			}
		}
		p.point = result; // 4
